...
(function(){
  ...
  function operationalLocked(date){return isFriday(date);}
...
const q=($('#trxSearch')?.value||'').toLowerCase(),t=$('#trxType')?.value||'',from=$('#trxFrom')?.value||'',to=$('#trxTo')?.value||'';let rows=state.rows.filter(r=>['pemasukan','pengeluaran','transfer','setoran_tabungan','penarikan'].includes(r.tipe));rows=rows.filter(r=>{const d=data(r),hay=[r.tgl,r.tipe,d.kategori,d.keterangan,d.no_transaksi,d.akun].join(' ').toLowerCase();return(!q||hay.includes(q))&&(!t||r.tipe===t)&&(!from||r.tgl>=from)&&(!to||r.tgl<=to)});$('#trxTable tbody').innerHTML=rows.map(r=>{const d=data(r);return `<tr><td>${r.tgl}</td><td>${esc(d.no_transaksi||'-')}</td><td><span class="pill">${typeLabel(r.tipe)}</span></td><td>${esc(d.kategori||'-')}</td><td>${esc(d.akun||'-')}</td><td class="money">${fmt(d.nominal)}</td><td>${esc(d.keterangan||'-')}</td><td><div class="row-actions"><button class="icon-btn edit-trx" data-id="${r.id}">Edit</button><button class="icon-btn danger-text del" data-id="${r.id}">Hapus</button></div></td></tr>`}).join('')||'<tr><td colspan="8" class="empty">Belum ada transaksi.</td></tr>';$$('#trxTable .edit-trx').forEach(b=>b.onclick=()=>{const r=state.rows.find(x=>x.id===b.dataset.id);if(r)openTransaction(r.tipe,r)});$$('#trxTable .del').forEach(b=>b.onclick=()=>deleteConfirm(b.dataset.id));}
  ...
  function renderDaily(){
    sortAllDateData();
const drow=state.rows.filter(r=>r.tipe==='harian').sort((a,b)=>b.tgl.localeCompare(a.tgl));const selected=$('#dailyDate');if(selected&&!selected.value)selected.value=isoToday();loadDailyForm(selected?.value||isoToday());$('#dailyTable tbody').innerHTML=drow.map(r=>{const d=data(r);return `<tr><td>${r.tgl}</td><td>${fmt(d.pend1)}</td><td>${fmt(d.pend2)}</td><td>${fmt(d.titip1)}</td><td>${fmt(d.titip2)}</td><td>${fmt(d.titip3)}</td><td>${fmt(d.tabungan)}</td><td class="money">${fmt(totalDaily(d))}</td><td><button class="icon-btn edit-daily" data-date="${r.tgl}">Edit</button> <button class="icon-btn danger-text del" data-id="${r.id}">Hapus</button></td></tr>`}).join('')||'<tr><td colspan="9" class="empty">Belum ada laporan harian.</td></tr>';$$('#dailyTable .edit-daily').forEach(b=>b.onclick=()=>{nav('harian');$('#dailyDate').value=b.dataset.date;loadDailyForm(b.dataset.date)});$$('#dailyTable .del').forEach(b=>b.onclick=()=>deleteConfirm(b.dataset.id));}
  function loadDailyForm(date){const r=state.rows.find(x=>x.tipe==='harian'&&x.tgl===date),d=data(r);['pend1','pend2','titip1','titip2','titip3'].forEach(k=>$('#'+k).value=num(d[k]));$('#dailySaving').value=num(d.tabungan);$('#dailyTotal').textContent=fmt(totalDaily({pend1:num(d.pend1),pend2:num(d.pend2),titip1:num(d.titip1),titip2:num(d.titip2),titip3:num(d.titip3),tabungan:num(d.tabungan)}));const locked=isFriday(date);$('#fridayNotice').textContent=locked?'Jumat otomatis libur — formulir dikunci.':'Hari operasional — laporan dapat disimpan.';$('#fridayNotice').className='notice '+(locked?'warn':'good');$$('#tab-harian input').forEach(i=>{if(i.id!=='dailyDate')i.disabled=locked});$('#saveDaily').disabled=locked;}
  async function saveDaily(){const date=$('#dailyDate').value;if(!date)return;if(isFriday(date))return toast('Hari Jumat otomatis libur.',true);const d={pend1:num($('#pend1').value),pend2:num($('#pend2').value),titip1:num($('#titip1').value),titip2:num($('#titip2').value),titip3:num($('#titip3').value),tabungan:num($('#dailySaving').value)};await writeRow(date,'harian',d,`harian:${date}`);toast('Laporan harian tersimpan.');await refresh();}
...
function sortChronological(rows,field='tgl'){return Array.isArray(rows)?[...rows].sort((a,b)=>dateKey(a?.[field])-dateKey(b?.[field])):rows;}
  ...
  // ============================================================
  // SORTIR SEMUA DATA BERDASARKAN TANGGAL
  // Aturan: tanggal paling awal di atas -> tanggal paling akhir di bawah.
  // ============================================================
  function universalDateKey(value){
    ...
    if(!Number.isNaN(iso) && /^\d{4}[-/]\d{1,2}[-/]\d{1,2}/.test(s)) return iso;
    ...
    // fallback for strings containing a recognizable date
  ...
  function sortByDateAscending(list, fields=['tgl','tanggal','date','created_at','createdAt','time']){
    if(!Array.isArray(list)) return list;
    return [...list].sort((a,b)=>{
      const av=fields.map(f=>a?.[f]).find(v=>v!==undefined && v!==null && v!=='');
      const bv=fields.map(f=>b?.[f]).find(v=>v!==undefined && v!==null && v!=='');
      const d=universalDateKey(av)-universalDateKey(bv);
      if(d!==0) return d;
      // Stable-ish secondary ordering by original insertion/id.
      const ai=String(a?.id ?? a?._id ?? '');
      const bi=String(b?.id ?? b?._id ?? '');
      return ai.localeCompare(bi);
    });
  }

  function sortAllDateData(){
    // Sort every top-level array in state that contains at least one date field.
    if(typeof state!=='undefined' && state && typeof state==='object'){
      Object.keys(state).forEach(key=>{
        const value=state[key];
        if(Array.isArray(value) && value.length && value.some(row=>{
          if(!row || typeof row!=='object') return false;
          return ['tgl','tanggal','date','created_at','createdAt','time'].some(f=>row[f]!==undefined && row[f]!==null && row[f]!=='');
        })){
          state[key]=sortByDateAscending(value);
        }
      });
    }
    return state;
